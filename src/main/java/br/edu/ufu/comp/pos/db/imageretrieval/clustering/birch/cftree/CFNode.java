...
public class CFNode {
	...
	private ArrayList<CFEntry> entries = null; // stores the CFEntries for this
												// node
	...
	public CFNode(int maxNodeEntries, double distThreshold, int distFunction,
			boolean applyMergingRefinement, boolean leafStatus) {
	...
	public CFEntryPair splitEntry(CFEntry closest) {
		...
		CFNode newNode1 = new CFNode(maxNodeEntries, distThreshold, distFunction,
				applyMergingRefinement, oldNode.isLeaf());
		newEntry1.setChild(newNode1);

		CFEntry newEntry2 = new CFEntry();
		CFNode newNode2 = new CFNode(maxNodeEntries, distThreshold, distFunction,
				applyMergingRefinement, oldNode.isLeaf());
	 ...
	 * @param newE2
	 */
	protected void redistributeEntries(ArrayList<CFEntry> oldEntries, CFEntryPair farEntries,
	 ...
	 * @param e2
	 */
	protected void redistributeEntries(ArrayList<CFEntry> oldEntries1,
			ArrayList<CFEntry> oldEntries2, CFEntryPair closeEntries, CFEntry newE1,
			CFEntry newE2) {
	 ...
	 */
	protected void redistributeEntries(ArrayList<CFEntry> oldEntries1,
			ArrayList<CFEntry> oldEntries2, CFEntry newE) {
		ArrayList<CFEntry> v = new ArrayList<CFEntry>();
		v.addAll(oldEntries1);
		v.addAll(oldEntries2);
	...
	public void mergingRefinement(CFEntryPair splitEntries) {
		...
		if ((oldNode1Entries.size() + oldNode2Entries.size()) > maxNodeEntries) {
			...
			// (the only exception is when the root is first split, but that's
			// treated separately)
			CFNode newNode = new CFNode(maxNodeEntries, distThreshold, distFunction,
					applyMergingRefinement, oldNode1.isLeaf());
