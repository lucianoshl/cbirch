 ...
 *
 */
public class CFTree implements ClusteringMethod, TreeNode {
     ...
     * final subclusters
     */
    public CFNode leafListStart = null;
     ...
     */
    public CFTree(int maxNodeEntries, double distThreshold, DistanceMeasure distFunction,long memoryLimit, boolean applyMergingRefinement) {
        ...
        this.setMemoryLimitMB(memoryLimit);
    ...
    public void rebuildTree() {
        ...
        logger.info("====== Rebuiding tree: start");
        ...
        logger.info("====== Rebuiding tree: end");
        copyTree(newTree);
    }
     ...
     */
    public double computeNewThreshold(CFNode leafListStart, DistanceMeasure distFunction, double currentThreshold) {
    ...
    private boolean hasReachedMemoryLimit(CFTree tree, long limit) {
        ...
        logger.info("Check Tree("+System.identityHashCode(this)+") size " + (Math.ceil(memory / Double.valueOf(limit) * 100)) + "%" + " " + humanReadableTreeSize
                + " ");
     ...
     */
    public CFTree rebuildTree(int newMaxEntries, double newThreshold, DistanceMeasure distFunction, boolean applyMergingRefinement,
                              boolean discardOldTree) {

        CFTree newTree = new CFTree( newMaxEntries, newThreshold, distFunction, this.getMemoryLimitMB(), applyMergingRefinement );
    ...
    public void finishBuild() {
        ...
        // "this.getEntriesAmount() > %s rebuilding", this.leavesLimit ) );
        // this.rebuildTree();
        // }
...
//    }
    ...
    public TreeNode build(Dataset dataset) {
        ...
        int totalFeatures = dataset.getTotalFeatures();

        logger.debug("Build CFTree: start");
        dataset.scanAllFeatures((sift,position) -> {
            logger.trace(String.format("%s/%s",totalFeatures,position));
            this.insertEntry(sift);
        });
        logger.debug("Build CFTree: end");
