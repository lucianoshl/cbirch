 ...
 *
 */
public class CFNode implements TreeNode {

    private static final String LINE_SEP = System.getProperty("line.separator");

    @Getter
    @Setter
    private CFEntry parent = null;

    public List<CFEntry> entries = null; // stores the CFEntries for
                                               // this
                                               // node
    ...
    private DistanceMeasure distFunction = new EuclideanDistance(); // the distance function to use

    private boolean leafStatus = false; // if true, this is a leaf

    public CFNode nextLeaf = null; // pointer to the next leaf (if not a leaf,
    ...
    public CFNode(int maxNodeEntries, double distThreshold, DistanceMeasure distFunction, boolean applyMergingRefinement,
            boolean leafStatus) {
    ...
    /**
     * 
     * @return true if this is only a place-holder node for maintaining correct
     *         pointers in the list of leaves
     */
    ...
    /**
     * 
     * @return the distance threshold used to decide whether a CFEntry can
     *         absorb a new entry
     */
    ...
    public DistanceMeasure getDistFunction() {
     ...
     * @return a positive integer, if the leaf entries were enumerated after
     *         data insertion is finished, otherwise -1
     */
     ...
     * @return TRUE if the new entry could be inserted without problems,
    ...
    public boolean insertEntry(CFEntry e) {
        ...
        if (closest.hasChild()) { // if closest has a child we go down with a
            ...
            this.addToEntryList(e);
            return true; // no split necessary at the parent level
        } else { // not enough space on this node
            this.addToEntryList(e); // adds it momentarily to this node
     ...
     * @param newE2
     */
    protected void redistributeEntries(List<CFEntry> oldEntries, CFEntryPair farEntries, CFEntry newE1,
                                       CFEntry newE2) {
    ...
    /**
     * Called when "merging refinement" is attempted but no actual merging can
     * be applied
     * 
     * @param oldEntries1
     * @param oldEntries2
     ...
     * @param e2
     */
    protected void redistributeEntries(List<CFEntry> oldEntries1, List<CFEntry> oldEntries2,
    ...
    /**
     * Called when "merging refinement" is attempted and two entries are
     * actually merged
     * 
     * @param oldEntries1
     * @param oldEntries2
    ...
    public void mergingRefinement(CFEntryPair splitEntries) {
        ...
        if ((oldNode1Entries.size() + oldNode2Entries.size()) > maxNodeEntries) {
            ...
            if (oldNode1.isLeaf() && oldNode2.isLeaf()) { // this is done to
                ...
                CFNode dummy = new CFNode(0, 0, distFunction, false, true);
    ...
    /**
     * Substitutes the entries in this node with the entries of the parameter
     * node
     * 
     * @param n
    ...
    public String toString() {
        ...
        return "CfNode[entries="+this.getEntries().size()+"]";
...
//        CFEntry closest = findClosestEntry(fakeEntry);
        ...
        Collections.sort(this.entries,(a,b)->{
            return Double.compare(this.distFunction.compute(fakeEntry.getCenter(),b.getCenter()),
                    this.distFunction.compute(fakeEntry.getCenter(),a.getCenter()));
        });

        double distance = Double.MAX_VALUE;
        CFEntry closest = null;
        int position = -1;
        for (CFEntry entry : this.entries){
            double computed = this.distFunction.compute(entry.getCenter(), fakeEntry.getCenter());
            if (computed < distance){
                distance = computed;
                closest = entry;
                position = this.entries.indexOf(entry);
            }
        }
