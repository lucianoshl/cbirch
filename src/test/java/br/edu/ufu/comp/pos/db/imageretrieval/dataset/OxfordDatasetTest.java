...
public class OxfordDatasetTest {
    ...
    private void validateSource(int limit, int vocabularySize, double map)
        ...
        Report.clear();
        double threshold = 5;
        callExperiment(limit, threshold);
        logger.debug(Report.get("vocabulary_size"));
        logger.debug(Report.get("map"));
        TestCase.assertEquals(vocabularySize, Report.get("vocabulary_size"));
        TestCase.assertEquals(map, Report.get("map"));
    }


    private void callExperiment(int limit, double threshold) {

        dataset.setScanLimit(limit);
        new Framework().run(dataset, new TreeFactory().createCFTree(50, threshold, 1024, 1000000), 4);
